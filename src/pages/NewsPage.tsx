...
export default function NewsPage() {
  ...
  if (loading) return <div className="min-h-screen bg-black flex items-center justify-center text-gray-500 font-bold">読み込み中...</div>;
      ...
      <div className="max-w-2xl mx-auto p-4 flex flex-col gap-4">
        ...
        {latest ? (
          <div className="animate-in fade-in duration-500">
            <div className="p-6 rounded-[32px] bg-white/5 border border-white/10 shadow-2xl mb-4">
              ...
              <div className="text-[16px] leading-relaxed text-gray-200 whitespace-pre-wrap mb-6">
                {latest.content}
              </div>

              <p className="text-[12px] leading-snug text-gray-600 mb-8">
            ...
            {!showHistory && news.length > 1 && (
              <div className="flex justify-center">
                <button 
                  ...
                  className="flex items-center gap-2 px-4 py-2 rounded-full border border-white/10 hover:bg-white/5 transition-all text-[13px] font-bold text-gray-500"
                >
        ...
        {showHistory && (
          <div className="mt-2 animate-in slide-in-from-bottom-2 duration-300">
            <div className="flex items-center justify-between mb-4 px-2">
              <span className="text-sm font-bold text-gray-400">過去のニュース</span>
              <button 
            ...
            <div className="flex flex-col gap-3">
              {historyItems.map((item) => (
                <div 
                  key={item.id} 
                  className="p-5 rounded-[24px] bg-white/5 border border-white/5"
                >
                  <div className="text-[11px] text-gray-500 mb-1">
                    {new Date(item.created_at).toLocaleDateString('ja-JP')}
                  </div>
                  <h3 className="text-md font-bold mb-1 tracking-tight">{item.title}</h3>
